...
def pandas_page():
    ...
    fig_bar = px.bar(df, x='State', y='Votes', color='Candidate', barmode='group', title="Votes per State")
    st.plotly_chart(fig_bar)

    # Pie chart for vote share by state
    fig_pie = px.pie(df, names='State', values='Votes', title="Vote Share by State")
    ...
    if 'State' in df.columns:
        ...
        fig_map.update_geos(fitbounds="locations", visible=False, projection=dict(type="mercator"))
